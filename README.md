...
| **라이브러리** | 채널별 추출 현황 카드, 폴더(그룹)로 채널 묶기·폴더 전체 영상 병합 보기, 영상 목록 필터(제목·카테고리·내용 검색), 자막 전문 열람·복사, 영상/채널 삭제 |
| **추출** | 채널·재생목록 URL 스캔 → 조건(최신 N개·기간·카테고리·멤버십·검색어) 선택 후 추출, 진행율 표시, 쿠키 상태 확인. 재생목록은 영상별 원채널 폴더에 저장되고 재생목록 제목이 카테고리로 추가됨 |
