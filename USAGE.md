...
### 무자막 영상 전사 (Whisper 폴백)

```bash
./yt.sh transcribe            # 전 채널의 무자막 영상을 음성 전사
./yt.sh transcribe 채널명 --limit 2
```

자막이 아예 없는 영상(추출 시 `⚠ 무자막`으로 남은 것)의 음성을 faster-whisper로
로컬 전사해 SRT/TXT를 만듭니다. 결과는 일반 자막과 똑같이 검토·인덱싱·질의에
쓰이고, 대시보드에는 `🎤전사` 뱃지로 표시됩니다.

- 영상당 수 분 걸립니다 (CPU 전사). 최초 1회 모델(기본 small) 다운로드.
- 모델 변경: `WHISPER_MODEL=medium ./yt.sh transcribe ...` (tiny/base/small/medium)
...
| **라이브러리** | 채널별 추출 현황 카드 · **폴더**(채널 그룹) · 영상 목록 필터(제목/카테고리/내용 검색) · 자막 전문 열람·복사 · **챕터 목록**(타임스탬프 링크) · **⬇ MD**(Markdown 내보내기, Obsidian 호환) · 영상/채널 삭제 |
| **추출** | 채널·재생목록 URL 스캔 → 조건 선택(최신 N개·기간·카테고리·멤버십 포함·제목 검색어) → 추출 실행 · 진행율 표시 · 쿠키 상태 확인. 진행/완료 통계의 **신규·수정·스킵·오류** 등 항목을 클릭하면 해당 영상 목록과 이유(오류 메시지·스킵 사유 등)가 표시됨 |

추출 탭 사용 흐름: URL 입력(또는 등록 채널 카드 클릭) → 스캔 완료 후 조건 화면에서 대상 미리보기 확인 → "선택 대상 추출 시작". 단일 영상 URL(`watch?v=...`)을 넣으면 스캔 없이 바로 추출됩니다.

**🔔 새 영상 확인**: 추출 탭의 "새 영상 확인" 버튼을 누르면 등록된 모든 채널의 RSS 피드를 확인해 아직 추출하지 않은 새 영상을 채널별로 보여줍니다 (자동 감시 아님 — 버튼을 누를 때만 확인). 채널 이름을 클릭하면 바로 그 채널의 추출 조건으로 이동합니다. RSS는 채널당 최근 15개까지만 제공됩니다.

재생목록 URL(`playlist?list=...`)도 지원합니다: 결과물은 각 영상의 **원채널 폴더**(`output/채널명/`)에 저장되고, 재생목록 제목이 그 채널의 카테고리로 추가되어 라이브러리 탭에서 재생목록 이름으로 필터링할 수 있습니다. 여러 채널이 섞인 재생목록이면 필요한 채널이 자동 등록되고, **재생목록 제목과 같은 이름의 폴더로 자동 분류**됩니다. (`watch?v=...&list=...` 형태는 단일 영상으로 처리됩니다.)

**채널 폴더**: 라이브러리 탭에서 채널 카드의 📁 버튼으로 채널을 폴더에 넣거나 뺄 수 있습니다 (같은 이름을 입력하면 같은 폴더로 묶임). 폴더는 헤더 클릭으로 접기/펼치기가 되고(처음엔 접힌 상태), 폴더 헤더의 **"전체 보기"**를 누르면 폴더 안 모든 채널의 영상이 최신순 하나의 목록으로 합쳐져 표시됩니다 (각 행에 원채널 표시, 자막 열람·삭제·**내용 검색**도 그대로 동작 — 검색은 폴더 안 모든 채널을 훑어 점수순으로 상위 결과를 보여줍니다). 추출 탭의 채널 현황도 같은 폴더로 묶여 표시됩니다. 폴더 정보는 `channels.yaml`의 `group` 필드에 저장됩니다.
...
- **이름 변경**: 라이브러리 탭의 ✏️ 버튼으로 채널 이름(폴더 이동 포함)·영상 제목(파일명은 유지)·카테고리(선택 후 ✏️)·폴더 이름을 바꿀 수 있습니다. 검색 인덱스 메타데이터도 함께 갱신됩니다. 추출 작업 중에는 변경이 거부됩니다.
