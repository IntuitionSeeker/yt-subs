...
**채널 폴더**: 라이브러리 탭에서 채널 카드의 📁 버튼으로 채널을 폴더에 넣거나 뺄 수 있습니다 (같은 이름을 입력하면 같은 폴더로 묶임). 폴더는 접기/펼치기가 되고, 폴더 헤더의 **"전체 보기"**를 누르면 폴더 안 모든 채널의 영상이 최신순 하나의 목록으로 합쳐져 표시됩니다 (각 행에 원채널 표시, 자막 열람·삭제도 그대로 동작). 폴더 정보는 `channels.yaml`의 `group` 필드에 저장됩니다.
